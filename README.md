...
* `node["jetty"]["ssl_pass"]` - set the password used to encrypt keys and keystore. randomly generated by default.
* `node["jetty"]["ssl_port"]` - set the HTTPS port to use. Set this property and `node["jetty"]["ssl_subject"]` to enable HTTPS. nil by default.
* `node["jetty"]["ssl_subject"]` - set the SSL subject for the SSL certificate, e.g. `/C=ACME/L=LonneyTown/O=AcmeInc/CN=www.acme.com/emailAddress=root@acme.com`. Set this property and `node["jetty"]["ssl_subject"]` to enable HTTPS. nil by default.
